...
| **会话列表附加字段（lastMessageContent / unreadCount）** | **`SessionResponse` 新增 `lastMessageContent`（最后一条消息内容预览，最多50字）和 `unreadCount`（当前用户在当前会话中的未读消息数）两个字段，由 `SessionService.listByTenant` / `listByUser` 在返回会话列表时填充** | **会话列表页除了需要展示"有哪些会话"外，往往还需要展示"最后说了什么"和"有没有新消息"，在 REST 列表接口里直接返回这两个值，避免前端为获取预览或未读数多发请求；`lastMessageContent` 查询时按 `createdAt DESC` 取最新一条消息的内容并截断至50字；`unreadCount` 调用 `MessageRepository.countUnreadBySessionAndReceiver` 按当前访问者身份（USER或TENANT）统计未读数；MVP 阶段每次查询单条会话各自产生1条额外SQL（N+1），数据量上来后可改为批量 `IN` 查询优化 |
